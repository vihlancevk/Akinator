...
const stackData_t RECALLOC_VALUE = (stackData_t)1488;
...
//{-------------------------------------------------------------------
    ...
    void Dump(stack_t *stack, const char *nameFunction, size_t line, const char *filePath)
    {
        ...
        if (stackError != STACK_DATA_NO_CREATE)
        {
            ...
            {
                if (((stackData_t*)((char*)stack->data + sizeof(uint64_t)))[i] == RECALLOC_VALUE)
                {
                    fprintf(foutput, "\t\t\t(%s) [%zu] - RECALLOC_VALUE\n", type, i);
                }
                else
                {
                    fprintf(foutput, "\t\t\t(%s) [%zu] - %s\n", type, i,
...
stack_t* StackReverse(stack_t *stack)
{
    ...
    int sizeBuf = stack->size;
    stackData_t buf[sizeBuf] = {};

    size_t i = 0;
    for (i = 0; i < sizeBuf; i++)
    {
        StackPop(stack, (stackData_t*)((char*)buf + i * sizeof(stackData_t)));
    }

    for (i = 0; i < sizeBuf; i++)
    {
        StackPush(stack, (stackData_t)buf[i]);
    }
